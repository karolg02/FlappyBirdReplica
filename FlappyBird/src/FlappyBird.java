...
public class FlappyBird extends JPanel implements ActionListener, KeyListener {
    ...
    FlappyBird() {
        ...
        // game timer (set, but not started until game begins)
    ...
    public void draw(Graphics g) {
        ...
        if (gameOver) {
            g.drawString("Game Over: " + (int) score, 85, 320);
        } else {
            g.drawString(String.valueOf((int) score), 10, 35);
        }

        if (!gameStarted) {
            g.drawString("Press SPACE to start", 30, 320);
        }
    }
    ...
    // actions performed 60 times per second
    @Override
